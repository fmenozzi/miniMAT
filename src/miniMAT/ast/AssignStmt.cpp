...
namespace miniMAT {
    namespace ast {
        AssignStmt::AssignStmt(std::shared_ptr<Reference> ref, std::shared_ptr<Expression> expr) 
        {
            this->ref  = ref;
            this->expr = expr;
        ...
        void AssignStmt::PrintResult(std::shared_ptr<std::map<std::string, ast::Matrix>> vars,
                                     ast::Matrix ans,
                                     bool suppressed) const 
        {
            if (ref->ClassName() == "IdRef") 
                util::PrintResult(ref->RefSpelling(), val, suppressed);
        }
    }
}
